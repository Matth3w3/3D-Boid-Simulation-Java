/** File: MRProject/BoidSimulation.java by Matthew Perales + Michael Perales
 * 
 */
...
public class BoidSimulation implements ActionListener, ChangeListener {
    ...
    public void start() {
        ...
        Clip line = null, line2 = null;
        try {
            // audioInput = AudioSystem.getAudioInputStream(BoidSimulation.class.getResource("C418 - Stal (Minecraft Volume Beta).wav"));
            // line2 = AudioSystem.getClip(); line2.open(audioInput);
            audioInput = AudioSystem.getAudioInputStream(BoidSimulation.class.getResource("C418 - Aria Math (Minecraft Volume Beta).wav"));
            line = AudioSystem.getClip(); line.open(audioInput);
        } catch (Exception e) { e.printStackTrace(); }
        line.start();
        // Robot robot = null;
        // try { robot = new Robot(); robot.mouseMove(width/2, height/2);
        ...
        while (frame.isShowing()) {
            // if (!line.isActive()) line2.start();
            // if (!line.isActive() && !line2.isActive()) line.start();
            if (frames == 0) previousTime = System.currentTimeMillis()*0.001;
            offscreen.setColor(new Color(0, 0, 50, 255));
            offscreen.fillRect(0, 0, width, height);
